...
class _MySplashScreenState extends State<MySplashScreen> {
  startTimer() {
    Timer(const Duration(seconds: 3), () async {
      // eğer satıcı zaten giriş yapmışsa, Ana Ekran'a yönlendir
      if (firebaseAuth.currentUser != null) {
        Navigator.push(
            context, MaterialPageRoute(builder: (c) => const HomeScreen()));
      }
      // eğer satıcı daha önce giriş yapmadıysa, Giriş Ekranına yönlendir
      else {
        Navigator.push(
  ...
  void initState() {
    super.initState();
    // sayfa oluşturulduğunda zamanlayıcıyı başlat
    startTimer();
  }
